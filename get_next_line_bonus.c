...
/*   Updated: 2023/11/29 12:58:58 by kait-baa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char		*line;
	static char	*buff_line[1024];

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	buff_line[fd] = help_get_next_line(fd, buff_line[fd]);
	if (!buff_line[fd])
		return (NULL);
	line = get_line_of_text(buff_line[fd]);
	buff_line[fd] = save_next_line(buff_line[fd]);
	return (line);
}
